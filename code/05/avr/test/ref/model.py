...
# Two full wraps of modulus 7. The card arms at phase 5, so the walk starts
# part-way round and an off-by-one shows up on the first edge.
REQ_EDGES = REQ_CARD[1] * 2
...
def write_req_vectors(path):
    ...
    """
    addr, modulus, released, arm_phase = REQ_CARD
    bits = step_case(modulus, set_bits(released), arm_phase, REQ_EDGES)
    path.write_text(
        "\n".join(
            [
                "# Generated by model.py. Do not edit.",
                "# req_walk.py loads this ring over I2C, arms, and then drives REQ.",
                f"address {addr:02x}",
                f"modulus {modulus}",
                "release " + ",".join(str(p) for p in released),
                f"arm {arm_phase}",
